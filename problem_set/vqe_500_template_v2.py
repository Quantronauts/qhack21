...
def find_excited_states(H):
    ...
    ### first 2 ###
    if 2*2==4:
        def cost_fn_first2(params):
            return 2.0*cost_fn0(params) + 1.0*cost_fn1(params)

        opt = qml.NesterovMomentumOptimizer(stepsize=0.1)

        for n in range(max_iterations):
            params, prev_energy = opt.step_and_cost(cost_fn_first2, params)
            energy = cost_fn_first2(params)
    ...
    if 2*2==4:
        ...
        #energies[0] = cost_fn0(params)
        #energies[1] = cost_fn1_alt(params)
