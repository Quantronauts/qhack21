...
def find_excited_states(H):
    ...
    def variational_ansatz1_alt(params, wires):
        n_qubits = len(wires)
        n_rotations = len(params)
    ...
    cost_fn1_alt = qml.ExpvalCost(variational_ansatz1_alt, H, dev)
    ...
    ### optimize params to get ground state in ansatz0 ###
    if 2*2==4:
        def cost_fn_ground(params):
            return cost_fn0(params)

        opt = qml.NesterovMomentumOptimizer(stepsize=0.1)

        for n in range(max_iterations):
            params, prev_energy = opt.step_and_cost(cost_fn_ground, params)
            energy = cost_fn_ground(params)
    ...
    if 2*2==4:
        ...
        for n in range(max_iterations):
            params, prev_energy = opt.step_and_cost(cost_fn_first, params)
            energy = cost_fn_first(params)
            conv = np.abs(energy - prev_energy)
            if n % 20 == 0:
                print('Iteration = {:},  Energy = {:.8f} Ha'.format(n, energy))
            if conv <= conv_tol:
    ...
    if 2*2==4:
        ...
        for n in range(max_iterations):
            params, prev_energy = opt.step_and_cost(cost_fn_second, params)
            energy = cost_fn_second(params)
            conv = np.abs(energy - prev_energy)
            if n % 20 == 0:
                print('Iteration = {:},  Energy = {:.8f} Ha'.format(n, energy))
            if conv <= conv_tol:
